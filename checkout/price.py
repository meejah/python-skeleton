...
def static(price):
    ...
    def static_pricer(**kw):
        return (price, price * kw['count'])
    static_pricer.__doc__ = '$%0.2f each' % (price / 100.0)
    directlyProvides(static_pricer, IPricer)

    return static_pricer
...
def cheap_after_dinner(price_before, price_after, dinnertime):
    ...
    '''
    def after_dinner_pricer(**kw):
        if datetime.today().hour >= dinnertime:
            return (price_before, price_after * kw['count'])
        return (price_before, price_before * kw['count'])
    after_dinner_pricer.__doc__ = "$%.2f before %02d00 hours, $%.2f after" %\
        (price_before / 100.0, dinnertime, price_after / 100.0)
    return after_dinner_pricer
...
def daily_special(price, day_of_week, discount):
    ...
    '''
    def daily_special_pricer(**kw):
        myprice = price
        if datetime.today().weekday() == day_of_week:
            myprice = int(price * discount)
        return (myprice, myprice * kw['count'])
    weekdays = ['mon', 'tue', 'wed', 'thrs', 'fri', 'sat', 'sun']
    daily_special_pricer.__doc__ = '$%.2f each, %d%% off %s' % \
...
def buy_n_get_m_free(price, n, m):
    ...
    def buy_n_pricer(**kw):
        ...
        total = paid * price
        return (None, total)
    buy_n_pricer.__doc__ = 'Buy %d get %d free' % (n, m)
